...
dataname = os.path.join(_data_dir, "Pantheon+SH0ES.dat")
covname = os.path.join(_data_dir, "Pantheon+SH0ES_STAT+SYS.cov")
...
def model_SNIa(u):
    ...
    cosmo = jc.Planck15(
        h=u[0], Omega_c=u[1], Omega_b=0, Omega_k=u[2], w0=u[3]
    )  # Barion mass doesn't matter for the expansion of the universe
...
def create_mock(fiducial_params, noise=True):
    ...
    if noise:
        noise_vector = jnp.array(
            np.random.multivariate_normal(mean=zero_vector, cov=covariance_pantheon)
        )
...
def create_ppd_chain(
    th1_samples, sample_size=10, n_jobs=4, cov_matrix=covariance_pantheon
):
    """
